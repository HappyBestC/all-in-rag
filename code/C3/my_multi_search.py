...
data_to_insert = []
for img_path in tqdm(image_list,desc="生成图像嵌入"):
    vector = encoder.encode_image(img_path)
    data_to_insert.append({"vector":vector, "image_path": img_path})
if data_to_insert:
...
query_text = "Chinese dragon"
# query_vector = encoder.encode_query(image_path=query_image_path, text=query_text)
query_vector = encoder.encode_text(text=query_text)  # 仅使用文本编码进行检索
